...
const SPEC_ID = process.env.POSTMAN_SPEC_ID ?? "1210b2ab-b12d-4a19-b1d5-cbf28da6eec2";
const COLLECTION_UID =
  process.env.POSTMAN_COLLECTION_UID ?? "42700275-5823a8cb-3329-4126-9fdc-6fb5e57cfd90";
const SPEC_FILE = process.env.POSTMAN_SPEC_FILE ?? "docs/openapi/auth-backend.v1.yaml";
const SPEC_REMOTE_PATH = process.env.POSTMAN_SPEC_REMOTE_PATH ?? "index.yaml";
const POLL_INTERVAL_MS = Number.parseInt(process.env.POSTMAN_TASK_POLL_MS ?? "2000", 10);
const TIMEOUT_MS = Number.parseInt(process.env.POSTMAN_TASK_TIMEOUT_MS ?? "90000", 10);
...
const request = async (method, path, body) => {
  ...
  if (!response.ok) {
    const detail = payload?.detail ?? payload?.error?.message ?? payload?.raw ?? "Request failed";
...
const syncOrIgnoreAlreadyInSync = async () => {
  ...
  try {
    ...
    if (error?.status === 400 && typeof detail === "string" && detail.includes("already in sync")) {
...
const encodeFilePath = (value) => value.split("/").map(encodeURIComponent).join("/");
...
const pollTask = async (taskUrl) => {
  ...
  while (Date.now() <= deadline) {
    ...
    if (status === "failed") {
      const details = task?.details ? JSON.stringify(task.details) : "unknown reason";
  ...
  throw new Error(`[postman-sync] Timeout waiting for task completion (${TIMEOUT_MS} ms).`);
};
...
const main = async () => {
  ...
  stdout(`[postman-sync] Updating spec file ${SPEC_REMOTE_PATH} on spec ${SPEC_ID}`);
  await request("PATCH", `/specs/${encodeURIComponent(SPEC_ID)}/files/${filePath}`, { content });

  stdout(`[postman-sync] Syncing collection ${COLLECTION_UID} from spec ${SPEC_ID}`);
