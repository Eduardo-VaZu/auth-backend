...
export function AnalysisTab({ data }: AnalysisTabProps) {
  ...
  if (Number.parseFloat(passRate) < 100) {
    recommendations.push(
      `Aumentar tasa de aprobación al 100% (actual: ${passRate}%)`,
    )
  }
  if (
    coverageRow &&
    parseMetric(coverageRow.statements) < thresholds.statements
  ) {
    recommendations.push(
      `Incrementar coverage de statements al ${thresholds.statements}% (actual: ${coverageRow.statements}%)`,
    )
  }
  if (coverageRow && parseMetric(coverageRow.branches) < thresholds.branches) {
    recommendations.push(
      `Mejorar coverage de branches al ${thresholds.branches}% (actual: ${coverageRow.branches}%)`,
    )
  }
  if (failedTests.length > 0) {
    recommendations.push(
      `Investigar y corregir ${failedTests.length} suite(s) fallida(s)`,
    )
  }
  if (lowCoverageRows.length > 0) {
    recommendations.push(
      `Priorizar tests en ${lowCoverageRows.length} archivo(s) con coverage < 20%`,
    )
  }
  if (recommendations.length === 0) {
    recommendations.push(
      'Todos los thresholds cumplidos. Considerar aumentar thresholds para mejorar calidad.',
    )
  }
    ...
    <section className="tab-panel">
      <section className="one-col">
        <article className="card section">
          ...
          <p className="lead">
            Evaluación crítica de la calidad de pruebas, coverage y hallazgos
            automáticos.
          </p>
          ...
          <div className="stack">
            <div className="callout">
              ...
              <div style={{ marginTop: 4, color: 'var(--muted)' }}>
                {passed} de {total} suites aprobadas
              </div>
            </div>
            ...
            <div className="callout">
              ...
              {lowCoverageRows.length === 0 ? (
                <div style={{ color: 'var(--ok)' }}>
                  No hay módulos con coverage crítico
                </div>
              ) : (
