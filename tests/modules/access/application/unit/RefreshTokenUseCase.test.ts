...
describe('RefreshTokenUseCase', () => {
  ...
  it('Rechazar al expirar Refresh Token', async () => {
    ...
    mockRefreshTokenRepo.findByJti.mockResolvedValue({
      ...defaultDbToken,
  ...
  it('Incidente de no coincidencia en el hasheo del token', async () => {
    ...
    await expect(
      useCase.execute({
        refreshToken: 'token-falso-o-robado',
        ipAddress: '127.0.0.1',
        userAgent: '',
        requestId: '',
      }),
    ...
    expect(mockAuthUnitOfWork.run).toHaveBeenCalled()
    expect(mockSessionStore.deleteAllRefreshTokens).toHaveBeenCalledWith(
      baseUserId,
    )
  })
    ...
    mockAuthUnitOfWork.run = vi.fn((cb) =>
      cb({
        ...
        refreshTokenRepository: {
          ...mockRefreshTokenRepo,
          revokeActiveByJti: vi.fn().mockResolvedValue(null),
        },
        authAuditService: { recordEvent: vi.fn() },
        acquireUserMutationLock: vi.fn(),
      }),
  ...
  it('Reuso explícito de un token ya revocado debe revocar todas las sesiones y tokens', async () => {
    ...
    mockRefreshTokenRepo.findByJti.mockResolvedValue({
      ...defaultDbToken,
      revokedAt: new Date(),
      replacedByTokenId: 'new-token-uuid',
    })
    ...
    expect(revokeAllTokens).toHaveBeenCalledWith({
      userId: baseUserId,
